...
All instances of modelMapper.map() were replaced with commentServiceMapperUtil.

Added two static utility methods:
commentServiceMapperUtil(Comment comment) for mapping Comment to CommentDto.
commentServiceMapperUtil(T source, Class<U> targetClass) for generic mapping between DTOs and entities.
