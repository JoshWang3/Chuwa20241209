...
3, To properly verify the certificate, need to import the server's certificate into Postman's trust store.
...
(1)Extract the certificate and private key from the JKS file:
keytool -exportcert -alias myapp -keystore myapp.jks -file myapp.crt
keytool -importkeystore -srckeystore myapp.jks -destkeystore myapp.p12 -deststoretype PKCS12

(2)Convert the PKCS12 (.p12) file to PEM format:

keytool -importkeystore -srckeystore myapp.jks -destkeystore myapp.p12 -srcstoretype JKS -deststoretype PKCS12
openssl pkcs12 -in myapp.p12 -out myapp.key -nocerts -nodes

download openssl:
https://wiki.openssl.org/index.php/Binaries
https://kb.firedaemon.com/support/solutions/articles/4000121705

"C:\Program Files\FireDaemon OpenSSL 3\bin\openssl" pkcs12 -in myapp.p12 -out myapp.key -nocerts -nodes

(3), Import .crt and .key files in Postman settings.
