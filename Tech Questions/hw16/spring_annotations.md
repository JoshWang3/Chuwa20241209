 ...
 * Used when you want to test your application end-to-end.
 */
@SpringBootTest
public class ApplicationTest {
    @Test
    void contextLoads() {
        // Ensures the Spring context loads properly
    }
}
 ...
 * Typically not needed when using @SpringBootTest or @WebMvcTest.
 * 
 * @ContextConfiguration
 * Loads a specific Spring configuration for tests.
 * Useful for integration tests without starting the entire application context.
 */
@ExtendWith(SpringExtension.class)
@ContextConfiguration(classes = MyConfig.class) // Load specific config if needed
...
/**
 * @MockBean
 * Creates and injects a mock of a Spring bean.
 * Used to mock dependencies in integration or slice tests
 */
@SpringBootTest
...
/**
 * @WebMvcTest
 * Loads only the @Controller layer for unit testing REST APIs.Does not load the full application context.
 */
...
/**
 * @DataJpaTest
 * Loads only the JPA layer (repositories and database).
 * Includes an in-memory database for testing purposes.
 */
@DataJpaTest
public class RepositoryTest {
    ...
    @Test
    void testRepository() {
        // Save and query data from the in-memory database
    }
}

/**
 * @TestConfiguration
 * Defines custom configurations for specific tests.
 */
@TestConfiguration
...
public class JUnit5LifecycleTest {
    ...
    static void beforeAllTests() {
        System.out.println("Before all tests - Initialize shared resources.");
    }
 ...
 * Enables Mockito in a JUnit 5 test class.
 * Best for pure unit tests that don’t rely on Spring's application context.
 * 
 * @Mock and @InjectMocks
 * @Mock: Creates a mock object.
 * @InjectMocks: Injects mocked dependencies into the object being tested.
 * These are used in combination with @ExtendWith(MockitoExtension.class) for pure Mockito tests.
 */
@ExtendWith(MockitoExtension.class)
public class PureUnitTest {
    ...
    public void testProcessData() {
        when(dependencyService.getData()).thenReturn("Mock Data");

        String result = mainService.processData();
 ...
 * Used to register the microservice with a service registry (e.g., Eureka).
 ...
 * @EnableGateway (Spring Cloud Gateway)
 * Used to enable API Gateway for routing requests between microservices.
 */
 ...
 * @FeignClient
 * Used to call another microservice via HTTP.
 */
 ...
 * @CircuitBreaker
 * Used to handle failures in microservices.
 */
...
public Order fallbackMethod(Long id, Throwable ex) {
 ...
 * Used to consume messages from Kafka or RabbitMQ.
 */
 ...
 * Used to secure microservices with OAuth2.
 */
@EnableResourceServer
@SpringBootApplication
